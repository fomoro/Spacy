...
if TYPE_CHECKING:
    from src.application.linguistic_evidence_mapper import LinguisticEvidenceBundle
...
class IntentResolver:
    ...
    Carga conjuntamente la definición de intenciones, los campos conversacionales
    y las reglas de acción, y rechaza referencias incoherentes al iniciar.
    ...
    def __init__(
        ...
        self._conversation_action_rules = self._load_related_resource(
        ...
        self._priorities = self._taxonomy_priorities(
    ...
    def _load_intent_definition(
        ...
        if isinstance(source, Mapping):
            ...
            path = Path(source)
            if path.is_dir():
                path = path / "intents_and_subintents.json"
            if not path.is_file():
                raise FileNotFoundError(
                    f"No existe la definición de intenciones del resolutor: {path}"
                )
            ...
            raise TypeError(
                "intents_and_subintents debe ser una ruta o un diccionario"
            )
        settings = data.get("resolver_settings")
        if not isinstance(settings, dict):
            raise ValueError(
                "intents_and_subintents.json debe contener 'resolver_settings'."
            )
        if not isinstance(settings.get("thresholds"), dict):
            raise ValueError("resolver_settings debe contener 'thresholds'.")
        if not isinstance(settings.get("source_multipliers"), dict):
            raise ValueError("resolver_settings debe contener 'source_multipliers'.")
        IntentResolver._validate_resolver_settings(settings)
        return data

    @staticmethod
    def _validate_resolver_settings(settings: Mapping[str, Any]) -> None:
        thresholds = settings["thresholds"]
        multipliers = settings["source_multipliers"]
        expected_thresholds = {
            "minimum_score",
        ...
        if set(thresholds) != expected_thresholds:
            raise ValueError(
                "resolver_settings.thresholds debe contener exactamente: "
                + ", ".join(sorted(expected_thresholds))
                + "."
            )
        if set(multipliers) != expected_multipliers:
            raise ValueError(
                "resolver_settings.source_multipliers debe contener exactamente: "
                + ", ".join(sorted(expected_multipliers))
                + "."
            )
        for name, value in {**thresholds, **multipliers}.items():
            if not isinstance(value, (int, float)) or isinstance(value, bool):
                raise ValueError(f"El parámetro '{name}' debe ser numérico.")
        if float(thresholds["minimum_score"]) < 0:
            raise ValueError("minimum_score no puede ser negativo.")
        if float(thresholds["clarification_margin"]) < 0:
            raise ValueError("clarification_margin no puede ser negativo.")
        if not 0 < float(thresholds["maximum_confidence"]) <= 1:
            raise ValueError("maximum_confidence debe estar entre 0 y 1.")
        if any(float(value) < 0 for value in multipliers.values()):
            raise ValueError("Los multiplicadores no pueden ser negativos.")

    @staticmethod
    def _load_related_resource(
        ...
        *,
        ...
        if selected is None and isinstance(resolver_source, (str, Path)):
            ...
            base_directory = resolver_path if resolver_path.is_dir() else resolver_path.parent
            selected = base_directory / filename
        ...
        if isinstance(selected, Mapping):
            ...
            raise ValueError(
                f"{filename} es obligatorio cuando las intenciones se entregan como diccionario."
            )
        else:
            raise TypeError(f"{filename} debe ser una ruta o un diccionario")
        return data

    @staticmethod
    def _taxonomy_pairs(taxonomy: Mapping[str, Any]) -> set[str]:
        intents = taxonomy.get("intents")
        if not isinstance(intents, Mapping) or not intents:
            raise ValueError(
                "intents_and_subintents.json debe contener un objeto 'intents' no vacío."
            )
        pairs: set[str] = set()
        for intent_id, intent in intents.items():
            if not isinstance(intent, Mapping):
                raise ValueError(f"La intención '{intent_id}' debe ser un objeto.")
            subintents = intent.get("subintents")
            if not isinstance(subintents, Mapping) or not subintents:
                raise ValueError(
                    f"La intención '{intent_id}' debe contener subintenciones."
                )
            pairs.update(
                f"{intent_id}.{subintent_id}" for subintent_id in subintents
            )
        return pairs

    @staticmethod
    def _taxonomy_priorities(taxonomy: Mapping[str, Any]) -> dict[str, int]:
        ...
        for intent_id, intent in taxonomy["intents"].items():
            ...
            if (
                ...
                raise ValueError(
                    f"La intención '{intent_id}' debe definir tie_break_priority como entero no negativo."
                )
            priorities[str(intent_id)] = priority
        return priorities

    def _validate_runtime_resources(self) -> None:
        errors: list[str] = []
        actions = self._conversation_action_rules.get("conversation_actions")
        rules = self._conversation_action_rules.get("rules_by_intent_and_subintent")
        questions = self._conversation_action_rules.get("questions")
        for key in (
            "conversation_actions",
            "rules_by_intent_and_subintent",
            "questions",
        ):
            if not isinstance(self._conversation_action_rules.get(key), dict):
                errors.append(
                    f"conversation_action_rules.json debe contener un objeto '{key}'."
                )
        if errors:
        ...
        for pair, raw_policy in rules.items():
            if pair not in self._valid_pairs:
                errors.append(f"Regla con intención o subintención desconocida: '{pair}'.")
            if not isinstance(raw_policy, Mapping):
                errors.append(f"La regla '{pair}' debe ser un objeto.")
                continue
            required = [str(slot) for slot in raw_policy.get("required_slots", [])]
            required_any = [
                [str(slot) for slot in group]
                for group in raw_policy.get("required_any", [])
                if isinstance(group, list)
            ]
            referenced_slots = set(required)
            for group in required_any:
                referenced_slots.update(group)
            unknown_slots = referenced_slots - self._valid_slots
            if unknown_slots:
                errors.append(
                    f"La regla '{pair}' utiliza campos desconocidos: "
                    + ", ".join(sorted(unknown_slots))
                    + "."
                )

            for mode_field in ("on_missing", "on_complete"):
                mode = raw_policy.get(mode_field)
                if mode is not None and mode not in actions:
                    errors.append(
                        f"La regla '{pair}' utiliza la acción desconocida '{mode}'."
                    )

            question_by_slot = raw_policy.get("question_by_slot", {})
            if not isinstance(question_by_slot, Mapping):
                errors.append(f"La regla '{pair}' debe contener question_by_slot.")
                continue
            ...
            if missing_question_slots:
                errors.append(
                    f"La regla '{pair}' no define preguntas para: "
                    + ", ".join(sorted(missing_question_slots))
                    + "."
                )
            referenced_questions = list(question_by_slot.values())
            for field in ("complete_question", "identity_question"):
                if raw_policy.get(field):
                    referenced_questions.append(raw_policy[field])
            unknown_questions = {
                str(question) for question in referenced_questions if question not in questions
            }
            if unknown_questions:
                errors.append(
                    f"La regla '{pair}' utiliza preguntas desconocidas: "
                    + ", ".join(sorted(unknown_questions))
                    + "."
                )

        if errors:
            raise ValueError("Configuración del resolutor inválida: " + " ".join(errors))
    ...
    def _add(self, scores, reasons, intent, subintent, weight, source, detail) -> None:
        ...
        if pair not in self._valid_pairs:
            raise ValueError(
                f"La evidencia contiene una intención o subintención no declarada: '{pair}'."
            )
    ...
    def _add_phrase_evidence(self, payload, scores, reasons) -> None:
        multiplier = float(self._multipliers.get("phrase_matcher", 1.0))
        for item in payload.get("phrase_matcher", {}).get("evidence", []):
            self._add(
                scores,
                reasons,
                item["intent"],
    ...
    def _add_entity_ruler_evidence(self, payload, scores, reasons) -> None:
        multiplier = float(self._multipliers.get("entity_ruler", 1.0))
        for item in payload.get("entity_ruler", {}).get("evidence", []):
            self._add(
                scores,
                reasons,
                item["intent"],
    ...
    def _available_slots(
        ...
        for context_key, slot in (
            ...
            if context.get(context_key):
                available.add(slot)
        return available.intersection(self._valid_slots)
    ...
    @property
    def resolver_settings(self) -> dict[str, Any]:
        return self._intents_and_subintents["resolver_settings"]
    ...
    @property
    def intents_and_subintents(self) -> dict[str, Any]:
        return self._intents_and_subintents

    @property
    def conversation_data_fields(self) -> dict[str, Any]:
        return self._conversation_data_fields
