...
def main() -> None:
    ...
    for case in dataset["cases"]:
        try:
            result = normalizer.normalize(case["message"])
            changed += int(result.normalized != case["message"])
            rows.append([
                case["id"], case["profile_id"], case["message"],
            ...
            errors += 1
            rows.append([case["id"], case["profile_id"], case["message"], "", json.dumps(["error"], ensure_ascii=False), str(exc)])
