...
from src.infrastructure import load_normalizer_config, TextNormalizer, PhraseMatcherService, MatcherService
...
def main() -> None:
    ...
    phrase_catalog_path = ROOT / "resources" / "phrase_matcher" / "catalog.json"
    matcher_catalog_path = ROOT / "resources" / "matcher" / "catalog.json"

    try:
        normalizer = TextNormalizer(load_normalizer_config(normalizer_path))
        phrase_matcher = PhraseMatcherService(phrase_catalog_path)
        matcher = MatcherService(matcher_catalog_path, phrase_matcher)
