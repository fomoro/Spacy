...
from .phrase_matcher_service import PhraseMatcherService, PhraseEntity
...
class MatcherService:
    ...
    def __init__(
        ...
        if isinstance(catalog_path, dict):
            self._catalog = catalog_path.get("matcher", catalog_path)
    ...
    def _load_catalog(path: Path) -> dict[str, Any]:
        if not path.exists():
            raise FileNotFoundError(f'No existe el catálogo Matcher: {path}')
        data = json.loads(path.read_text(encoding='utf-8'))
        data = data.get("matcher", data)
        if not isinstance(data.get('patterns'), list):
            raise ValueError("El catálogo debe contener una lista 'patterns'.")
        return data
    ...
    def analyze(self, text: str) -> MatcherResult:
        ...
        doc, phrase_entities = self._build_annotated_doc(text)
    ...
    def _build_annotated_doc(self, text: str) -> tuple[Doc, list[PhraseEntity]]:
        doc = self._nlp.make_doc(text)
        phrase_result = self._phrase_matcher.match(text)
        spans: list[Span] = []
