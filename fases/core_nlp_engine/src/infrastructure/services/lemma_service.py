...
class LemmaService:
    ...
    def __init__(
        ...
        if isinstance(catalog_path, dict):
            self._catalog = catalog_path.get("lemmas", catalog_path)
        ...
        self._ignored_pos = set(
            self._catalog.get("metadata", {}).get("ignored_pos", [])
        )
        self._minimum_token_length = int(
            self._catalog.get("metadata", {}).get("minimum_token_length", 1)
        )
        self._build_indexes()
    ...
    def _load_catalog(path: Path) -> dict[str, Any]:
        if not path.exists():
            raise FileNotFoundError(f"No existe el catálogo de lemas: {path}")
        data = json.loads(path.read_text(encoding="utf-8"))
        data = data.get("lemmas", data)
        if not isinstance(data.get("signals"), list):
            raise ValueError("El catálogo debe contener una lista 'signals'.")
        return data
