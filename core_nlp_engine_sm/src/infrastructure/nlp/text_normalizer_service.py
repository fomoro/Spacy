"""Normalización lingüística controlada para español colombiano."""
...
class TextNormalizerService:
    ...
    def _load_config(source: str | Path | Mapping[str, Any]) -> dict[str, Any]:
        ...
        section = data.get("normalizer", data)
        if not isinstance(section, dict) or not isinstance(section.get("options"), dict):
            raise ValueError("La configuración debe contener la sección 'normalizer'.")
        return section
